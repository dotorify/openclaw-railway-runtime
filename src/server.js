...
const ENABLE_LEGACY_SETUP = process.env.ENABLE_LEGACY_SETUP?.toLowerCase() === "true";

const OPENCLAW_CONTROL_TOKEN = process.env.OPENCLAW_CONTROL_TOKEN?.trim();
if (!OPENCLAW_CONTROL_TOKEN) {
  console.warn(
    "[control-api] OPENCLAW_CONTROL_TOKEN is not set. /control endpoints will be disabled.",
  );
}
...
const ENABLE_WEB_TUI =
  ENABLE_LEGACY_SETUP && process.env.ENABLE_WEB_TUI?.toLowerCase() === "true";
...
function saveConfigRaw(nextConfig) {
  const p = configPath();
  fs.mkdirSync(path.dirname(p), { recursive: true });
  const backupPath = `${p}.bak.${Date.now()}`;
  try {
    if (fs.existsSync(p)) fs.copyFileSync(p, backupPath);
  } catch (err) {
    console.warn(`[control-api] failed to write backup: ${err.message}`);
  }
...
function requireControlAuth(req, res, next) {
  if (!OPENCLAW_CONTROL_TOKEN) {
    return res.status(503).json({ ok: false, error: "control-api disabled" });
  }
  ...
  const expectedHash = crypto
    .createHash("sha256")
    .update(OPENCLAW_CONTROL_TOKEN)
    .digest();
  const ok =
    tokenHash.length === expectedHash.length &&
    crypto.timingSafeEqual(tokenHash, expectedHash);
  if (!ok) return res.status(403).json({ ok: false, error: "invalid token" });
  return next();
}

function maskSecrets(obj) {
  const secretKeyRx = /(token|secret|password|apiKey|api_key|key)$/i;
  if (obj === null || obj === undefined) return obj;
  if (Array.isArray(obj)) return obj.map(maskSecrets);
  if (typeof obj === "object") {
    const out = {};
    for (const [k, v] of Object.entries(obj)) {
      if (secretKeyRx.test(k) && typeof v === "string" && v.length > 0) {
        out[k] = "***";
      } else {
        out[k] = maskSecrets(v);
      }
...
// Control API (used by external UI service)
app.get("/control/status", requireControlAuth, async (_req, res) => {
  let openclawVersion = null;
  try {
    openclawVersion = (await runCmd(OPENCLAW_NODE, clawArgs(["--version"]))).output
      .trim();
  } catch {}
  ...
  res.json({
    ...
    paths: { stateDir: STATE_DIR, workspaceDir: WORKSPACE_DIR, configPath: configPath() },
    version: { openclaw: openclawVersion },
  });
});

app.get("/control/config/summary", requireControlAuth, async (_req, res) => {
  ...
  const discordConfigured = !!cfg.channels?.discord?.token;

  res.json({
    ok: true,
    configured: true,
    channels: { discord: { configured: discordConfigured } },
    agents: maskSecrets(agents),
    bindings: maskSecrets(bindings),
  });
});

app.get("/control/config", requireControlAuth, async (_req, res) => {
  if (!isConfigured()) return res.status(404).json({ ok: false, error: "not configured" });
  const cfg = loadConfigRaw();
  res.json({ ok: true, config: maskSecrets(cfg) });
});

app.post("/control/validate", requireControlAuth, async (req, res) => {
  try {
    ...
    // basic sanity checks
    if (next.agents?.list && !Array.isArray(next.agents.list)) {
      return res.json({ ok: false, errors: ["agents.list must be an array"] });
    }
...
app.patch("/control/config", requireControlAuth, async (req, res) => {
  try {
    const patch = req.body?.patch;
    if (!patch || typeof patch !== "object") {
      return res.status(400).json({ ok: false, error: "missing patch" });
...
app.post("/control/apply", requireControlAuth, async (_req, res) => {
  try {
    const r = await restartGateway();
    res.json({ ok: true, gateway: r });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
});
